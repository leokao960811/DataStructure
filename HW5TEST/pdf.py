...
def generate_pdf(text: str = None, df: pd.DataFrame = None) -> str:
    ...
    if df is not None:
        ...
        if "|" in text:
            ...
            parsed_df = table.parse_markdown_table(table_part)
            if parsed_df is not None:
                html_table=table.create_table(parsed_df)
            else:
    ...
    pdf_filename = fr"D:\Programmed Files\Python\TEST\HW5TEST\report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
